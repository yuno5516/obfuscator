...
/// \todo Notify the linker somehow that it should erase jumptable pointers too
namespace analysis::bb_decomp {
    ...
    void Instance<Img>::collect_jumptable_entries() {
        ...
        for (auto& [rva, info] : jump_tables_) {
            ...
            for (std::size_t i = 0; true; i++) {
                ...
                /// \todo Check for function bounds (if its even possible)
    ...
    void Instance<Img>::expand_jumptables() {
        for (auto& [rva, info] : jump_tables_) {
            ...
            /// Remove the base
            /// \fixme Do we really need to always remove it though?
            ...
            for (auto it = info.entries.begin(); it != info.entries.end(); std::advance(it, 1)) {
                ...
                /// Save new bb info
                new_bbs.emplace_back(new_bb, first_node, last_node, 2); // \fixme hardcoded count
