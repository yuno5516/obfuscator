...
/// \todo split this monstrosity to multiple files
namespace analysis {
    ...
    // Instruction repr
    // \todo Some smart .destroy method that would unlink zasm node, remove relocs, etc
    struct insn_t {
    ...
    struct bb_t {
        ...
        std::shared_ptr<insn_t> push_insn(zasm::Node* insn_node_ptr, const bb_provider_t* bb_provider, const std::optional<rva_t> rva = std::nullopt,
            ...
            if (ref == nullptr) {
                if (insn_node_ptr->holds<zasm::Label>()) { // \todo issue some sort of warning?
                    push_label(insn_node_ptr, bb_provider);
                }
                return nullptr; // \fixme this should be handled somehow different
            }
            ...
            /// \todo We can probably fill the successors/predecessors list from here?
            auto update_cf = [&it, &ref, &bb_provider, this]() -> void {
                ...
                auto push_not_met = [is_conditional, it, bb_provider, push_cf_changer, reschedule, this]() -> void {
                    ...
                    /// \todo Make sure that this insn is at the very start of the basic block
    ...
    [[nodiscard]] inline std::shared_ptr<bb_t> insn_t::linear_successor() const {
        ...
        /// \todo Check if last insn within the BB, return next insn if so
